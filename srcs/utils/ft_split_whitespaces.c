...
/*   Updated: 2024/12/14 14:38:21 by macbook          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *allocate_and_copy_whitespace(const char *s, int startindex, int count)
{
    ...
    if (subarray == NULL)
        return (NULL);
    memcpy(subarray, &s[startindex], count);
    subarray[count] = '\0';
    return (subarray);
}
...
char **str_div_logic_whitespace(char **parentarray, const char *s, int ar_index)
{
    int i;
    int count;
    int startindex;
    ...
    while (s[i] != '\0')
    {
        if (!ft_is_whitespace(s[i]))
        {
            ...
            while (s[i] != '\0' && !ft_is_whitespace(s[i++]))
                count++;
            parentarray[ar_index] = allocate_and_copy_whitespace(s, startindex, count);
...
char **ft_split_whitespace(char const *s)
{
    ...
    ar_index = 0;
    parentarray = (char **)malloc(sizeof(char *) * (countrows_whitespace(s) + 1));
    if (parentarray == NULL)
    {
        return (NULL);
    }
